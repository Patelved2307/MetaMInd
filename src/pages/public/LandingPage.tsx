  ...
  return (
    <div className="bg-black text-white min-h-screen selection:bg-white/20 selection:text-white">
      ...
      <section className="min-h-screen overflow-hidden relative flex flex-col justify-between">
        ...
        <header className="relative z-20 px-6 py-6 w-full">
          <div className="liquid-glass rounded-full max-w-5xl mx-auto px-6 py-3 flex items-center justify-between shadow-2xl">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg bg-[#8DD3FF]/15 border border-[#8DD3FF]/30 flex items-center justify-center">
                <Sparkles className="w-4 h-4 text-[#8DD3FF]" />
              </div>
              <span className="text-white font-semibold text-lg tracking-tight font-display">Aether Learn</span>
        ...
        {/* Hero Main Content */}
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 py-12 text-center -translate-y-[8%] md:-translate-y-[12%] space-y-6 max-w-4xl mx-auto">
          {/* Tag */}
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full liquid-glass text-xs font-medium text-[#8DD3FF] border border-[#8DD3FF]/30">
            <Sparkles className="w-3.5 h-3.5" />
            <span>AI-Powered Adaptive Education</span>
          </div>

          {/* Main Heading */}
          <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl text-white tracking-tight whitespace-nowrap font-serif">
            Know it then <em className="italic">all</em>.
          </h1>

          {/* Input Form */}
          <form onSubmit={handleStartDoubt} className="max-w-xl w-full">
            <div className="liquid-glass rounded-full pl-6 pr-2 py-2.5 flex items-center gap-3 shadow-2xl border border-white/15 focus-within:border-[#8DD3FF]/50 transition-colors">
              <input
                type="text"
                value={doubtInput}
                onChange={(e) => setDoubtInput(e.target.value)}
                placeholder="Ask any doubt... e.g. 'I don't understand SQL JOINs'"
                className="w-full bg-transparent text-white placeholder:text-white/40 text-sm outline-none border-none"
              />
              <button
                type="submit"
                className="bg-[#8DD3FF] text-[#05070A] font-semibold rounded-full px-4 py-2.5 hover:bg-[#a6deff] active:scale-95 transition-all flex items-center gap-1.5 shrink-0 text-xs cursor-pointer"
              >
          ...
          {/* Subtitle */}
          <p className="text-white text-sm leading-relaxed max-w-lg px-4 opacity-90">
            Don't just get an answer. Understand what you need to learn next. Aether transforms questions into adaptive concept maps and gap analysis.
          </p>

          {/* Action Buttons */}
          <div className="flex flex-wrap items-center justify-center gap-4 pt-2">
            <Link to="/app/dashboard">
              <button className="liquid-glass rounded-full px-8 py-3 text-white text-sm font-medium hover:bg-white/10 transition-colors shadow-lg cursor-pointer">
                Enter Platform App
              </button>
            </Link>
          </div>
        ...
        <div className="relative z-10 flex justify-center gap-4 pb-12">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="liquid-glass rounded-full p-4 text-white/80 hover:text-white hover:bg-white/10 transition-all flex items-center justify-center"
          >
